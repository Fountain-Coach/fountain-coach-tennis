...
const server = createServer(async (request, response) => {
  ...
  const isQueryApp = new URL(request.url || '/', 'http://127.0.0.1').searchParams.get('app') === '1';
  const isApp = isQueryApp || pathname === '/app' || pathname.startsWith('/app/');
  const isTeatro = !isApp && (pathname === '/teatro' || pathname.startsWith('/teatro/'));
  const relative = (isTeatro ? pathname.replace(/^\/teatro/, '') : isApp ? pathname.slice('/app'.length) : pathname).replace(/^\/+/, '') || 'index.html';
