...
function endTime(time) {
  ...
  const total = hours * 60 + minutes + Number(configuration.matchDurationMinutes || 60);
...
function render() {
  ...
  const matchCount = schedule.reduce((total, day) => total + day.matches.length, 0);
  $('#metrics').innerHTML = schedule.length ? `<div class="metric"><strong>${schedule.length}</strong><span>Spieltage</span></div><div class="metric"><strong>${matchCount}</strong><span>Spiele</span></div><div class="metric"><strong>${currentErrors.length}</strong><span>Abweichungen</span></div>` : '';
}
